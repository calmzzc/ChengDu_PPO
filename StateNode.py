...
class StateNode:
    def __init__(self, state, step, line, agent, episode, train_flag, train_model):
        self.line = line  # 区间
        ...
        self.prob = np.array(0).reshape(1)  # 动作的prob
        self.val = np.array(0).reshape(1)  # 动作的val
    ...
    def get_action(self):  # 选择动作
        self.action, self.prob, self.val = self.agent.choose_action(self.state)
        self.action = np.array(self.action).reshape(1)
        self.prob = np.array(self.prob).reshape(1)
        self.val = np.array(self.val).reshape(1)
